...
- 多工作：工作资料、颜色、手动时薪输入、时薪历史、默认班次、交通费、时间取整、工资日与深夜加薪模板。
- 班次：新增、编辑、复制、软删除、每周重复、默认单日日期联动、显式跨日模式、计划/实际记录、休息、奖金与扣减。
- 冲突与风险：跨工作时间冲突阻止保存；顶部错误摘要可跳转查看冲突记录；周合计超过用户上限时二次确认；计划与实际数据分开保留。
- 日历：首页摘要以及月、周、日、年轻量视图；月历日期可进入日详情并基于所选日期新增。
- 收入：日、周、月、年、自定义范围，工作筛选，计划/实际来源，工资分项、工作图表和到账差额记录。
- 本地数据：SwiftData 离线保存；JSON 备份/恢复；恢复前自动创建受文件保护的本机备份；UTF-8 BOM CSV 导出。
- 系统服务：本地班次通知；由 App 管理的独立系统日历单向同步；不会修改其他日历事件。
...
本次交付验证环境：Xcode 26.6、iPhone 17 Pro / iOS 26.5。App 和全部测试目标编译成功；17 项单元测试及 5 项 UI 测试通过。

## 权限用途

- 通知：用户主动开启后，在班次开始前发送本地提醒。
...
- 重复系列的“本次及以后/整个系列”批量编辑、多段休息的可视化编辑、工资日/班次结束确认通知仍建议在下一迭代补齐。
